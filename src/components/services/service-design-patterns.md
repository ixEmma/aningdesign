...
| `flatSurfaces: true`                    | Flat (no-gradient) surfaces | Adds `.service-page--flat`, which replaces gradient **surfaces** with solid/tonal colour: the page + hero backgrounds, the mid-CTA panel, and the shared 145° card gradient. Does **not** recolour the primary button — that stays the shared gradient for cross-page consistency; pair with `solidPrimaryCta` for a gradient-free button. Opt-in and isolated. |
| `solidPrimaryCta: true`                 | Solid (gradient-free) primary button | Adds `.service-page--solid-primary`, which renders the primary CTA (`.service-primary-action`) as solid brand green and neutralises the timeline number-badge gradient — page-scoped only. Use with `flatSurfaces` for design pages that must be fully gradient-free without changing the shared gradient primary on other pages. |
| `heroArtwork` / `contentSections[].gallery` | Art-directed portfolio grid | Data-driven image slots: `{ featured: {src,alt,width,height,caption?,eager?}, supporting: [ … ] }` renders a featured image + supporting image row (`.service-artwork-grid`), preserving aspect ratio with lazy-loading and explicit dimensions. `heroArtwork` places it in a split hero (banner crop); `contentSections[].gallery` (+ optional `galleryCta`) places it inside a content section. Swap the image objects later without touching layout/CSS. |
| `heroArtworkStyle: 'collage'`           | Portrait hero collage | Renders `heroArtwork` as an uncropped portrait composition (`.service-hero-collage`): one featured design + up to two supporting designs, natural aspect ratios, restrained solid frames. Image objects accept `srcSet` / `sizes`. Suited to portrait social/campaign artwork the banner grid would crop. First used on `social-media-design`. |
| `contentSections[].showcase`            | Selected-proof grid | An uncropped 3-up proof grid inside a content section (`.service-section-showcase` → `.service-brand-showcase` reused as a controlled grid, not masonry). Each item `{ image: {src,srcSet,sizes,width,height,alt}, heading, caption }`; add `galleryCta` for a proof CTA. Lets a proof section sit between use-cases and process. Category label goes in `heading` (e.g. `Campaign design`), one factual line in `caption`. |
| `examples[].linkLabel`                  | Showcase card link text | Overrides the default "View example" link text on a showcase card (e.g. `View the social media portfolio`). Backward-compatible. |
| `audienceIntro`                         | Audience supporting line | Optional short line rendered under the audience heading (via the section `intro`). Keep it factual and brief. |
